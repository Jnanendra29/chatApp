...
const registerUser = async (req, res) => {
    ...
    try {
        ...
        if (user) return res.status(400).json("User already exists, please try another email username...");
        if (!name || !email || !password) return res.status(400).json("All fields are required...");
        if (!validator.isEmail(email)) return res.status(400).json("Please enter a valid email...");
        if (!validator.isStrongPassword(password)) return res.status(400).json("Please enter a Strong Password like Smg@1349...");
